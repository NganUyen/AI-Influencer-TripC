...
class TelegramSkillSessionStore:
    ...
    def _init_redis(cls) -> None:
        ...
        if Redis is None:
            logger.warning("Redis client not installed. Falling back to in-memory skill sessions.")
            cls._redis_enabled = False
            return

        redis_url = getattr(settings, "REDIS_URL", None)
        if not redis_url:
            logger.warning("REDIS_URL is not configured. Falling back to in-memory skill sessions.")
            cls._redis_enabled = False
            return
    ...
    async def get_session(cls, chat_id: int | str) -> Optional[SkillSession]:
        ...
        if cls._redis_enabled and cls._redis_client is not None:
            try:
                ...
                logger.warning("Redis read failed for skill session store: %s", exc)
                cls._redis_enabled = False
    ...
    async def set_session(cls, chat_id: int | str, session: SkillSession) -> None:
        ...
        if cls._redis_enabled and cls._redis_client is not None:
            try:
                ...
                logger.warning("Redis write failed for skill session store: %s", exc)
                cls._redis_enabled = False
